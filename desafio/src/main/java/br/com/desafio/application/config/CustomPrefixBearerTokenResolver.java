...
public class CustomPrefixBearerTokenResolver implements BearerTokenResolver {
    private static final Pattern AUTHORIZATION_PATTERN =
            Pattern.compile(SecurityConstants.AUTHORIZATION_PATTERN, Pattern.CASE_INSENSITIVE);
    ...
    private String resolveFromAuthorizationHeader(HttpServletRequest request) {
        ...
        if (!StringUtils.startsWithIgnoreCase(authorization, SecurityConstants.TOKEN_PREFIX)) return null;
        Matcher matcher = AUTHORIZATION_PATTERN.matcher(authorization);
        if (!matcher.matches()) {
            BearerTokenError error = BearerTokenErrors.invalidToken("Bearer token is malformed");
            throw new OAuth2AuthenticationException(error);
        }
        return matcher.group(SecurityConstants.GROUP_PATTERN);
    }
