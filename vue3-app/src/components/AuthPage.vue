...
const formStore = ref({ username: '', email: '', password: '', cpf:'' })
        ...
        <div class="col-md-6 offset-md-3 col-xs-12">
          ...
          <p class="text-xs-center">
            <router-link :to="isRegister ? '/login' : '/register'">
              {{ isRegister ? 'Já tem uma conta' : 'Criar uma conta?' }}
            </router-link>
          </p>
          ...
          <form autocomplete="on" @submit.prevent="() => onSubmit()">
            <fieldset v-if="isRegister" class="form-group">
              <input
                v-model="formStore.username" required type="text" name="username" placeholder="Nome"
            ...
            <fieldset class="form-group" v-if="isRegister"> 
              <input
                v-model="formStore.cpf" required type="number" name="cpf" placeholder="CPF"
                class="form-control form-control-lg"
              >
            </fieldset>            
            <fieldset class="form-group">
              <input
                v-model="formStore.password" required name="password" type="password" placeholder="Senha"
                class="form-control form-control-lg"
              >
            </fieldset>
